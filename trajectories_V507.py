...
if simCase == "paraclete":
   ...
   xlimValue, ylimValue = 1.0, 2.00 # vorticella strange
else:
...
def plotPopVsIteration(nIterations,mu0, ptsX, ptsY):
    ...
    xAxis = 200
    yAxis = ylimValue * 0.8
    ax[0].text(xAxis*0.8,yAxis,r'$\mu_{0} = $' + "{:.3f}".format(mu0),fontsize=12)
    ax[0].scatter(nIterations,ptsX,color = 'k' , s = 8 , marker = '.')
    minorLocatorX = AutoMinorLocator(5) # number of minor intervals per major # inteval
    minorLocatorY = AutoMinorLocator(5)
    ax[0].set_ylabel("$x_{n}$",size = 16)
    ...
    ax[0].set_ylim([-0.05,xlimValue * 1.02])
    ax[0].grid(True)
    
    # the second subplot
    # shared axis X
    ax[1].scatter(nIterations,ptsY,color = 'k', s = 8, marker = '.')
    ax[1].set_xlabel("$n$",size = 16)
    ax[1].set_ylabel("$y_{n}$",size = 16)
    ...
    ax[1].set_ylim([-0.05,ylimValue * 1.02])
    ax[1].grid(True)
    #plt.show()
    plt.savefig(str(folderPopVsN)+"PopVsN"+"_{:.5f}".format(mu0)+".jpg")
    plt.close()
...
def plotFigurePhaseSpace(XorY, mu0, ptsX, ptsY):
    ...
    plt.grid(True)
    if XorY == "XPhaseSpace": 
        plt.savefig(str(folderXPhaseSpace)+"XPhaseSpace"+"_{:.5f}".format(mu0)+".jpg")
    else:
        plt.savefig(str(folderYPhaseSpace)+"YPhaseSpace"+"_{:.5f}".format(mu0)+".jpg")
    plt.close()
...
for mu_ in mu: 
    ...
    while (i < nIterations - 1):
        try:
            ...
            exception = True
            if x_next > 0:
                print("Warning Line 264! x_next positive infinite! x_next = {} at mu_ = {}".format(x_next,mu_))
                break
            else:
                print("Warning Line 267! x_next negative infinite! x_next = {} at mu_ = {}".format(x_next,mu_))
                break
            if y_next > 0:
                print("Warning Line 270! y_next positive infinitle! y_next = {} at mu_ = {}".format(y_next,mu_))
                break
            else:
                print("Warning Line 273! y_next negative infinite! y_next = {} at mu_ = {}".format(y_next,mu_))
                break
        else:
            ...
            if x_next < 0 :
                exception = True
                print("Warning Line 280! x_next negative x_next = {} at mu_ = {}".format(x_next, mu_))
                break
            if y_next < 0 and abs(y_next) < tolerance:
                y_next = 0.0
            if y_next < 0:
                exception = True
                print("Warning Line 286! y_next negative y_next = {} at mu_ = {}".format(y_next, mu_))
                break
            traceX[i+1] = x_next
            traceY[i+1] = y_next
            i += 1
...
#%%
# plot of plot of |omega1| vs mu0 and |omega0| vs mu0 for fixed points E1, E2, and E3
...
fig.savefig(str(folderEigVals)+"absEval_vs_mu0.jpg")
...
# legend (leg)
#ax[3,0]=plt.subplot(gs_[9])
ax[3,0].legend(handles1, labels1, title="Type")
ax[3,0].axis('off')

# remove frames in the empty ax
#ax[3,1]=plt.subplot(gs_[10])
ax[3,1].axis('off')
#ax[3,2]=plt.subplot(gs_[11])
ax[3,2].axis('off')

fig.savefig(str(folderEigVals)+"ImagVsRe_AbsVals.jpg")
...
for mu_ in mu: 
    ...
    while (i < nIterations - 1):
        try:
            ...
            exception = True
            if x_next > 0:
                print("Warning Line 300! x_next positive infinite! x_next = {} at mu_ = {}".format(x_next,mu_))
                break
            else:
                print("Warning Line 303! x_next negative infinite! x_next = {} at mu_ = {}".format(x_next,mu_))
                break
            if y_next > 0:
                print("Warning Line 306! y_next positive infinitle! y_next = {} at mu_ = {}".format(y_next,mu_))
                break
            else:
                print("Warning Line 309! y_next negative infinite! y_next = {} at mu_ = {}".format(y_next,mu_))
                break
        else:
            if x_next < 0 and abs(x_next) < tolerance:
                x_next = 0.0
            if x_next < 0 :
                exception = True
                print("Warning Line 316! x_next negative x_next = {} at mu_ = {}".format(x_next, mu_))
                break
            if y_next < 0 and abs(y_next) < tolerance:
                y_next = 0.0
            if y_next < 0:
                exception = True
                print("Warning Line 322! y_next negative y_next = {} at mu_ = {}".format(y_next, mu_))
                break
            traceX[i+1] = x_next
            traceY[i+1] = y_next
            i += 1 
...
#plt.show()
fig.savefig(str(folderTraj)+"Trajectory_all.jpg")
# end of plot coordinate space (phase portrait) V506
#%%
# plot phase spaces
...
for mu_ in mu: 
    ...
    while (i < nIterations - 1):
        try:
            ...
            exception = True
            if x_next > 0:
                print("Warning Line 634! x_next positive infinite! x_next = {} at mu_ = {}".format(x_next,mu_))
                break
            else:
                print("Warning Line 637! x_next negative infinite! x_next = {} at mu_ = {}".format(x_next,mu_))
                break
            if y_next > 0:
                print("Warning Line 640! y_next positive infinitle! y_next = {} at mu_ = {}".format(y_next,mu_))
                break
            else:
                print("Warning Line 643! y_next negative infinite! y_next = {} at mu_ = {}".format(y_next,mu_))
                break
        else:
            if x_next < 0 and abs(x_next) < tolerance:
                x_next = 0.0
            if x_next < 0 :
                exception = True
                print("Warning Line 650! x_next negative x_next = {} at mu_ = {}".format(x_next, mu_))
                break
            if y_next < 0 and abs(y_next) < tolerance:
                y_next = 0.0
            if y_next < 0:
                exception = True
                print("Warning Line 656! y_next negative y_next = {} at mu_ = {}".format(y_next, mu_))
                break
            traceX[i+1] = x_next
            traceY[i+1] = y_next
            i += 1 
    if exception == True: 
        ...
        diffTraceX = [ (traceX[i+1]-traceX[i]) for i in range( len(traceX) -1 ) ]
        plotFigurePhaseSpace("XPhaseSpace",mu_, traceX[0:-1], diffTraceX)                   

        ax.plot(traceX[0:-1],diffTraceX,'.',color=cmap.to_rgba(mu_),alpha=alpha_,markersize=3)  # for continuous color map
...
#plt.ylim(-0.01,1.02)
#plt.show()
fig.savefig(str(folderXPhaseSpace)+"phaseSpaceX_all.jpg")
#%%
...
for mu_ in mu: 
    ...
    while (i < nIterations - 1):
        try:
            ...
            exception = True
            if x_next > 0:
                print("Warning Line 703! x_next positive infinite! x_next = {} at mu_ = {}".format(x_next,mu_))
                break
            else:
                print("Warning Line 706! x_next negative infinite! x_next = {} at mu_ = {}".format(x_next,mu_))
                break
            if y_next > 0:
                print("Warning Line 709! y_next positive infinitle! y_next = {} at mu_ = {}".format(y_next,mu_))
                break
            else:
                print("Warning Line 712! y_next negative infinite! y_next = {} at mu_ = {}".format(y_next,mu_))
                break
        else:
            if x_next < 0 and abs(x_next) < tolerance:
                x_next = 0.0
            if x_next < 0 :
                exception = True
                print("Warning Line 719! x_next negative x_next = {} at mu_ = {}".format(x_next, mu_))
                break
            if y_next < 0 and abs(y_next) < tolerance:
                y_next = 0.0
            if y_next < 0:
                exception = True
                print("Warning Line 725! y_next negative y_next = {} at mu_ = {}".format(y_next, mu_))
                break
            traceX[i+1] = x_next
            traceY[i+1] = y_next
            i += 1 
    if exception == True: 
        ...
        diffTraceY = [ (traceY[i+1]-traceY[i]) for i in range( len(traceY) -1 ) ]
        plotFigurePhaseSpace("YPhaseSpace",mu_, traceY[0:-1], diffTraceY )   

        ax.plot(traceY[0:-1],diffTraceY,'.',color=cmap.to_rgba(mu_),alpha=alpha_,markersize=3)  # for continuous color map
...
#plt.ylim(-0.01,1.02)
#plt.show()
fig.savefig(str(folderYPhaseSpace)+"phaseSpaceY_all.jpg")
...
clip.write_videofile(str(folderTraj)+'trajectory.mp4')
...
clip.write_videofile(str(folderPopVsN)+'popVsnIterations.mp4')

image_files = [os.path.join(folderXPhaseSpace,img)
               for img in sorted(os.listdir(folderXPhaseSpace))
               if img.endswith(".jpg")]
clip = moviepy.video.io.ImageSequenceClip.ImageSequenceClip(image_files[0:-1], fps=fps)
clip.write_videofile(str(folderXPhaseSpace)+'phaseSpaceX.mp4')
